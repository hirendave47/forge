...
import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@earendil-works/forge-coding-agent";
import { Container, hyperlink, Text } from "@earendil-works/forge-tui";
...
async function fetchAdvisoryMetadata(forge: ExtensionAPI, cwd: string, target: string): Promise<GhMetadata | undefined> {
	const advisoryRef = parseAdvisoryUrl(target) ?? (await readAdvisoryRefFromDraft(cwd, target));
	if (!advisoryRef) return undefined;

	try {
		const result = await forge.exec("gh", [
			"api",
...
async function fetchGhMetadata(
	...
	if (kind === "advisory") {
		return fetchAdvisoryMetadata(forge, cwd, target);
	}
	...
	try {
		const result = await forge.exec("gh", args);
...
export default function promptUrlWidgetExtension(forge: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
		...
		const currentName = forge.getSessionName()?.trim();
		if (!currentName) {
			forge.setSessionName(desiredName);
			return;
		}
	...
	const updatePromptContext = (ctx: ExtensionContext, match: PromptMatch) => {
		...
		void fetchGhMetadata(forge, match.kind, match.target, ctx.cwd).then((meta) => {
	...
	forge.on("session_switch", async (_event, ctx) => {
