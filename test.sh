...
test_root="$(mktemp -d "$temp_parent/forge-test.XXXXXX")"
...
touch "$test_root/.forge-test-owned" "$test_root/npm-userconfig" "$test_root/npm-globalconfig"
...
cleanup() {
	...
	case "$test_root" in
		"$temp_parent"/forge-test.*)
			if [[ -d "$test_root" && ! -L "$test_root" && -f "$test_root/.forge-test-owned" ]]; then
				rm -rf -- "$test_root"
			else
...
test_env=(
	"PATH=$PATH:${HOME:-}/.forge/agent/bin"
	"PWD=$PWD"
	"HOME=$test_root/home"
