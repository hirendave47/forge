...
async function checkDirtyRepo(
	...
	const { stdout, code } = await forge.exec("git", ["status", "--porcelain"]);
...
export default function (forge: ExtensionAPI) {
	forge.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(forge, ctx, action);
	});

	forge.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(forge, ctx, "fork");
	});
}
