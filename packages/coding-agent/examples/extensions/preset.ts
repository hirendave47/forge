 ...
 * - ~/.forge/agent/presets.json (global)
 * - <cwd>/.forge/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `forge --preset plan` - start with plan preset
...
import type { Api, Model } from "@earendil-works/forge-ai";
import type { ExtensionAPI, ExtensionContext } from "@earendil-works/forge-coding-agent";
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@earendil-works/forge-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@earendil-works/forge-tui";
...
export default function presetExtension(forge: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: forge.getThinkingLevel(),
				tools: forge.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			forge.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = forge.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				forge.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await forge.setModel(originalState.model);
				}
				forge.setThinkingLevel(originalState.thinkingLevel);
				forge.setActiveTools(originalState.tools);
			} else {
				forge.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	forge.registerShortcut(Key.ctrlShift("u"), {
	...
	forge.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = forge.getFlag("preset");
	...
	forge.on("turn_start", async () => {
		if (activePresetName) {
			forge.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
