...
Example extensions for forge-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
forge --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.forge/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `forge.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs forge theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `forge.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using forge-ai's built-in Anthropic/OpenAI streaming via proxy |
