...
export default function (forge: ExtensionAPI) {
	forge.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await forge.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[forge] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await forge.exec("git", ["add", "-A"]);
		const { code: commitCode } = await forge.exec("git", ["commit", "-m", commitMessage]);
