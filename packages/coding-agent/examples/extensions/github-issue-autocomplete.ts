...
async function resolveGitHubRepo(forge: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await forge.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (forge: ExtensionAPI): void {
	forge.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(forge, ctx.cwd);
		if (!resolvedRepo.ok) {
