 ...
 *
 * Demonstrates forge.sendUserMessage() for sending user messages from extensions.
 * Unlike forge.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (forge: ExtensionAPI) {
	...
	forge.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			forge.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	forge.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				forge.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	forge.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				forge.sendUserMessage(args, { deliverAs: "followUp" });
