 ...
 * to the LLM. Custom entries are stored in the session via forge.appendEntry() and
 * rendered in interactive mode via forge.registerEntryRenderer().
 *
...
export default function (forge: ExtensionAPI) {
	forge.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	forge.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			forge.appendEntry<StatusCardData>("status-card", {
