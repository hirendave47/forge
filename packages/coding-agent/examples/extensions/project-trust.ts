 ...
 *
 *   mkdir -p ~/.forge/agent/extensions
 *   cp packages/coding-agent/examples/extensions/project-trust.ts ~/.forge/agent/extensions/
 *
 * Or:
 *
 *   forge -e packages/coding-agent/examples/extensions/project-trust.ts
 *
 * Try it in a project containing .forge, AGENTS.md/CLAUDE.md, or .agents/skills.
 */

import type { ExtensionAPI, ProjectTrustEventResult } from "@earendil-works/forge-coding-agent";
...
export default function (forge: ExtensionAPI) {
	...
	forge.on("project_trust", async (event, ctx): Promise<ProjectTrustEventResult> => {
