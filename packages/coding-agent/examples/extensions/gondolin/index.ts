 ...
 *
 * Runs forge's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   forge -e /path/to/forge/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (forge: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `forge ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	forge.registerCommand("gondolin", {
	...
	forge.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
