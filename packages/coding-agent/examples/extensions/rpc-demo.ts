...
export default function (forge: ExtensionAPI) {
	...
	forge.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "forge RPC Demo (new session)" : "forge RPC Demo");
	...
	forge.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	forge.on("turn_end", async (_event, ctx) => {
	...
	forge.registerCommand("rpc-input", {
	...
	forge.registerCommand("rpc-editor", {
	...
	forge.registerCommand("rpc-prefill", {
