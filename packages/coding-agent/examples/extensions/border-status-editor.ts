...
import type { Component, EditorTheme, TUI } from "@earendil-works/forge-tui";
...
export default function (forge: ExtensionAPI) {
	...
	forge.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await forge.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = forge.getThinkingLevel();
