...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@earendil-works/forge-coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@earendil-works/forge-tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@earendil-works/forge-tui";
...
export default function (forge: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like forge-doom would need)
	forge.registerCommand("overlay-animation", {
	...
	forge.registerCommand("overlay-anchors", {
	...
	forge.registerCommand("overlay-margins", {
	...
	forge.registerCommand("overlay-stack", {
	...
	forge.registerCommand("overlay-overflow", {
	...
	forge.registerCommand("overlay-edge", {
	...
	forge.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	forge.registerCommand("overlay-maxheight", {
	...
	forge.registerCommand("overlay-sidepanel", {
	...
	forge.registerCommand("overlay-toggle", {
	...
	forge.registerCommand("overlay-passive", {
	...
	forge.registerCommand("overlay-focus", {
	...
	forge.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/forge/packages/coding-agent/src/modes/interactive"
...
// Animation demo - proves overlays can handle real-time updates like forge-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (forge-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
