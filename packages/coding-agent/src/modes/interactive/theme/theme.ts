...
import type { ThinkingLevel } from "@earendil-works/forge-agent-core";
import {
	type EditorTheme,
	getCapabilities,
	Markdown,
...
import defaultDarkTheme from "./dark.json" with { type: "json" };
import defaultLightTheme from "./light.json" with { type: "json" };
...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		let darkTheme: ThemeJson = defaultDarkTheme as ThemeJson;
		let lightTheme: ThemeJson = defaultLightTheme as ThemeJson;
		try {
			...
			// Fall back to bundled default themes if reading from disk fails
		}
		BUILTIN_THEMES = {
			dark: darkTheme,
			light: lightTheme,
...
export function getAvailableThemesWithPaths(): ThemeInfo[] {
	...
	for (const name of Object.keys(getBuiltinThemes())) {
		...
		addTheme({ name, path: fs.existsSync(themePath) ? themePath : undefined });
	}
...
const THEME_KEY = Symbol.for("@earendil-works/forge-coding-agent:theme");
const THEME_KEY_OLD = Symbol.for("@earendil-works/forge-coding-agent:theme");
