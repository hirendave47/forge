...
import { Markdown, type MarkdownTheme } from "@earendil-works/forge-tui";
...
const DEFAULT_INSTALLER_API_BASE = "https://forge.dev/api/installer/releases";
...
function getActiveManagedInstallRoot(): string | undefined {
	const configuredRoot = process.env.FORGE_MANAGED_INSTALL_ROOT?.trim();
	...
	try {
		...
		if (marker.kind !== "forge-managed-install" || marker.schemaVersion !== 1 || marker.layout !== "releases-v1") {
			throw new Error();
		}
	} catch {
...
async function fetchInstallerArtifact(url: string, label: string): Promise<string> {
	const response = await fetch(url, { headers: { "User-Agent": getForgeUserAgent(VERSION) } });
	if (!response.ok) {
...
async function runManagedSelfUpdate(managedRoot: string, version: string): Promise<void> {
	...
	try {
		cleanupManagedStaging(managedRoot);
		const installerApiBase = (process.env.FORGE_INSTALLER_API_BASE?.trim() || DEFAULT_INSTALLER_API_BASE).replace(
			/\/+$/,
			"",
		);
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|forge] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update forge, installed packages, or model catalogs.
...
Short forms:
  ...
  ${APP_NAME} update forge             Update forge only (self works as alias to forge)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "forge";
			if (sourceIsSelf) {
