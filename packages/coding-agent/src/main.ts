...
import { type ImageContent, modelsAreEqual } from "@earendil-works/forge-ai";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.FORGE_OFFLINE);
	if (offlineMode) {
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `forge update` drain naturally instead.
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.FORGE_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: FORGE_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
