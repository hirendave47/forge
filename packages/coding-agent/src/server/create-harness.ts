...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as FORGE_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.FORGE_SESSION_ID = metadata.id;
						execution.env.FORGE_SESSION_FILE = sessionFile ?? "";
						execution.env.FORGE_PROVIDER = model.provider;
						execution.env.FORGE_MODEL = model.id;
						execution.env.FORGE_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
