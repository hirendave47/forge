...
import { Container, Text, truncateToWidth } from "@earendil-works/forge-tui";
...
export const DANGEROUS_PATTERNS = [
	...
	{
		regex: /\bkill\s+(-9|-s\s+KILL|-KILL|-s\s+9|-SIGKILL)\s+1\b|\bkill\s+1\b|\bkillall\s+(init|systemd)\b/i,
		reason: "Killing PID 1 / init / systemd is blocked for system stability.",
	},
	{
		regex: /\b(mkfs(\.\w+)?|wipefs|fdisk|parted|gdisk)\b|\bdd\s+.*of=\/dev\/(sd[a-z]|nvme\d+n\d+|vd[a-z]|hd[a-z]|mapper\/)/i,
		reason: "Disk format / partition wiping commands are blocked for data safety.",
	},
	{
		regex: /\b(iptables|ip6tables|nft)\s+.*(-F|--flush)\b|\bufw\s+disable\b|\bip\s+link\s+set\s+\S+\s+down\b/i,
...
export function createShellToolDefinition(
	...
	return {
		...
		async execute(
			...
			const safetyViolation = checkDangerousCommand(command);
			if (safetyViolation) {
				return {
					content: [
						{
							type: "text",
							text: `[SECURITY GUARDRAIL BLOCKED]: Command execution denied.\nReason: ${safetyViolation}\nCommand: ${command}`,
						},
					],
					isError: true,
					details: undefined,
