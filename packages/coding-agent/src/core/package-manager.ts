...
import { type ForgeManifest, readForgeManifest } from "./forge-manifest.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.FORGE_OFFLINE;
...
export class DefaultPackageManager implements PackageManager {
	...
	private collectPackageResources(
		...
		const manifest = readForgeManifest(join(packageRoot, "package.json"));
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof ForgeManifest];
