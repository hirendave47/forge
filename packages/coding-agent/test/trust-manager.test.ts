...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".forge", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".forge", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".forge", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".forge"), { recursive: true });
			writeFileSync(join(cwd, ".forge", "settings.json"), "{}");
