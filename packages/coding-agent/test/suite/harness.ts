...
import type { AgentMessage, AgentTool } from "@earendil-works/forge-agent-core";
...
import { registerFauxProvider, streamSimple } from "@earendil-works/forge-ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `forge-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
