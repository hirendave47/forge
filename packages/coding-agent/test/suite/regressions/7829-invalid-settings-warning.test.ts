import { Container } from "@earendil-works/forge-tui";
...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.FORGE_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.FORGE_OFFLINE;
			else process.env.FORGE_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});
