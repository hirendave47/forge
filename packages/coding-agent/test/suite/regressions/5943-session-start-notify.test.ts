import { fauxAssistantMessage } from "@earendil-works/forge-ai";
import { Container, Text } from "@earendil-works/forge-tui";
...
describe("regression #5943: session_start transient UI", () => {
	...
	it("subscribes before replacement session_start handlers send user messages", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(forge) => {
					forge.on("session_start", () => {
						forge.sendUserMessage("user from start");
					});
				},
			],
		});
	...
	it("runs the reload render hook before reload session_start handlers can notify", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(forge) => {
					forge.on("session_start", (event, ctx) => {
