...
describe("issue #8423 extension factory failure", () => {
	it("discards runtime changes and disables the failed API", async () => {
		...
		await loadExtensionFromFactory(
			(forge) => forge.registerProvider("working-provider", providerConfig),
			process.cwd(),
			eventBus,
			runtime,
			"<working>",
		...
		await expect(
			loadExtensionFromFactory(
				(forge) => {
					capturedApi = forge;
					forge.events.on("factory-failure", () => {
						eventCalls++;
					});
					forge.registerFlag("failed-flag", { type: "boolean", default: true });
					flagDuringLoad = forge.getFlag("failed-flag");
					forge.unregisterProvider("working-provider");
					forge.registerProvider("failed-provider", providerConfig);
