...
/**
 * Regression test for https://github.com/hirendave47/forge/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "forge-2791-"));
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, FORGE_CODING_AGENT_DIR: agentDir },
