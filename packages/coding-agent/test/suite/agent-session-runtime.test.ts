...
import { fauxAssistantMessage, fauxToolCall, registerFauxProvider } from "@earendil-works/forge-ai/compat";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `forge-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(forge: ExtensionAPI) => {
						forge.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((forge: ExtensionAPI) => {
			forge.on("message_end", (event) => {
	...
	it("settles the active response before session replacement", async () => {
		...
		const { runtime, faux } = await createRuntimeForTest((forge: ExtensionAPI) => {
			forge.registerTool({
				name: "block",
				label: "Block",
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `forge-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("duplicates the current active branch in-memory when forking at the current position", async () => {
		const tempDir = join(
			tmpdir(),
			`forge-runtime-suite-in-memory-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		mkdirSync(tempDir, { recursive: true });
	...
	it("updates the runtime session cwd on cross-cwd session replacement", async () => {
		const firstDir = join(tmpdir(), `forge-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `forge-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);
