...
import type { OAuthCredentials } from "@earendil-works/forge-ai";
import { getModel, streamSimple } from "@earendil-works/forge-ai/compat";
import { builtinProviders } from "@earendil-works/forge-ai/providers/all";
...
// ============================================================================
// OAuth API key resolution from ~/.forge/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".forge", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.forge/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.forge/agent/auth.json
 */
...
export const FORGE_AGENT_DIR = join(homedir(), ".forge", "agent");

/**
 * Get an AuthStorage instance backed by ~/.forge/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `forge-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
