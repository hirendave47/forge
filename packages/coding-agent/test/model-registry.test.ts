...
import { getApiProvider, getSupportedThinkingLevels } from "@earendil-works/forge-ai/compat";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `forge-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
