...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.FORGE_OFFLINE;
		delete process.env.FORGE_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.FORGE_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .forge skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .forge", async () => {
			const extDir = join(tempDir, ".forge", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".forge", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".forge", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".forge", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".forge", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".forge", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .forge/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-forge", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-forge"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-forge\ndescription: user forge\n---\n");
		...
		it("should use the project .forge dir as baseDir for project .forge skills", async () => {
			const projectBaseDir = join(tempDir, ".forge");
			const skillPath = join(projectBaseDir, "skills", "project-forge", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-forge"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-forge\ndescription: project forge\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".forge", "agent");
		...
		it("should dedupe user skill entries when ~/.forge/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .forge auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".forge\n");

			const skillDir = join(tempDir, ".forge", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.forge\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".forge", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the forge-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "forge-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .forge settings base", () => {
			...
			const rel = relative(join(tempDir, ".forge"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "forge-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "forge-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:forge-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:forge-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".forge"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".forge", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".forge", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".forge", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".forge", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".forge", "npm", "node_modules", "example");
