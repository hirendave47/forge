> forge can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@earendil-works/forge-tui`](https://github.com/hirendave47/forge/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@earendil-works/forge-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `FORGE_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@earendil-works/forge-tui";
...
Import from `@earendil-works/forge-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@earendil-works/forge-tui";
```
...
```typescript
import { matchesKey, Key } from "@earendil-works/forge-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@earendil-works/forge-tui";
...
```typescript
import { getMarkdownTheme } from "@earendil-works/forge-coding-agent";
import { Markdown } from "@earendil-works/forge-tui";
...
Set `FORGE_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
FORGE_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@earendil-works/forge-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@earendil-works/forge-coding-agent";
import { DynamicBorder } from "@earendil-works/forge-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@earendil-works/forge-tui";
...
```typescript
import { BorderedLoader } from "@earendil-works/forge-coding-agent";

forge.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@earendil-works/forge-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@earendil-works/forge-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@earendil-works/forge-tui";

forge.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while forge is streaming a response.
...
// Restore forge's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@earendil-works/forge-tui";
