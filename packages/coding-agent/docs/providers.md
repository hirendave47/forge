...
Forge supports subscription-based providers via OAuth and API key providers via environment variables or auth file. Built-in catalogs ship with forge; configured providers may refresh newer catalogs and cache them in `~/.forge/agent/models-store.json` for offline use.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.forge/agent/auth.json` and auto-refresh when expired. OpenRouter instead mints a user-controlled API key that does not expire automatically.
...
Radius is a dynamic `forge-messages` gateway. `/login radius` stores OAuth tokens in `auth.json`; the gateway catalog is refreshed independently and cached in `models-store.json`. Custom Radius gateways can be declared in `models.json` with `"oauth": "radius"` and a gateway `baseUrl`.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/hirendave47/forge/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/hirendave47/forge/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.forge/agent/auth.json`:
...
API key credentials can also include provider-scoped environment values. These values are used before process environment variables when resolving the credential key, provider/model headers, and provider configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `FORGE_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
forge --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
forge --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
forge --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
forge --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```

Forge automatically sets `x-session-affinity` for [prefix caching](https://developers.cloudflare.com/workers-ai/features/prompt-caching/) discounts.
...
Forge supports the llama.cpp router server. Configure it with `/login llama.cpp`, manage loaded models with `/llama`, and select a loaded model with `/model`.
