> forge can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to forge's agent capabilities. Use it to embed forge in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @earendil-works/forge-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@earendil-works/forge-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `forge.sendMessage()`.
...
The `Agent` class (from `@earendil-works/forge-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.forge/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.forge/extensions/`)
- Project skills:
  - `.forge/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.forge/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.forge/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@earendil-works/forge-ai";
import { ModelRuntime } from "@earendil-works/forge-coding-agent";

const modelRuntime = await ModelRuntime.create();

// create() restores cached catalogs but does not refresh them from forge.dev by default.
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.forge/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `FORGE_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "@earendil-works/forge-ai";
import { createAgentSession, ModelRuntime } from "@earendil-works/forge-coding-agent";

// Default: uses ~/.forge/agent/auth.json and ~/.forge/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any forge-ai CredentialStore
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@earendil-works/forge-coding-agent";
...
The `edit` tool returns `details.diff` for Forge's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@earendil-works/forge-coding-agent";
...
import { createAgentSession, defineTool } from "@earendil-works/forge-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `forge.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `forge.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.forge/agent/extensions/`, `.forge/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@earendil-works/forge-coding-agent";
...
**Event Bus:** Extensions can communicate via `forge.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/forge-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/forge-coding-agent";
...
1. Global: `~/.forge/agent/settings.json`
2. Project: `<cwd>/.forge/settings.json`
...
// Auth and Models
ModelRuntime // implements forge-ai Models and owns credential storage
