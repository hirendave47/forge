...
Extensions can register custom model providers via `forge.registerProvider()`. This enables:
...
Extensions can register either a complete forge-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Forge composes `models.json` overrides above registered native providers.
...
export default function (forge: ExtensionAPI) {
  forge.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  forge.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. forge waits for the factory before startup continues, so the provider is available during interactive startup and to `forge --list-models`.
...
Use `forge.unregisterProvider(name)` to remove a provider that was previously registered via `forge.registerProvider(name, ...)`:
...
// Later, remove it
forge.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map forge levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
forge.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@earendil-works/forge-ai";
...
Credentials are persisted in `~/.forge/agent/auth.json`:
...
- [anthropic-messages.ts](https://github.com/hirendave47/forge/blob/main/packages/ai/src/api/anthropic-messages.ts) - Anthropic Messages API
- [mistral-conversations.ts](https://github.com/hirendave47/forge/blob/main/packages/ai/src/api/mistral-conversations.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/hirendave47/forge/blob/main/packages/ai/src/api/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/hirendave47/forge/blob/main/packages/ai/src/api/openai-responses.ts) - OpenAI Responses API
- [google-generative-ai.ts](https://github.com/hirendave47/forge/blob/main/packages/ai/src/api/google-generative-ai.ts) - Google Generative AI
- [bedrock-converse-stream.ts](https://github.com/hirendave47/forge/blob/main/packages/ai/src/api/bedrock-converse-stream.ts) - AWS Bedrock
...
When a request exceeds the model's context window, forge can recover automatically by compacting the conversation and retrying. This recovery only kicks in if forge recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of forge's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/hirendave47/forge/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message forge does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase forge recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
export default function (forge: ExtensionAPI) {
  ...
  forge.on("message_end", (event, ctx) => {
...
`message_end` runs before forge tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what forge checks. With this in place, forge will:
...
- Match a provider-specific pattern, not forge's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of forge's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/hirendave47/forge/tree/main/packages/ai/test):
