...
```
~/.forge/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.forge/agent/sessions/`.

Forge also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, forge uses the `trash` CLI to avoid permanent deletion.
...
Source on GitHub ([forge-mono](https://github.com/hirendave47/forge)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/hirendave47/forge/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/hirendave47/forge/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/hirendave47/forge/blob/main/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/hirendave47/forge/blob/main/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/@earendil-works/forge-coding-agent/dist/` and `node_modules/@earendil-works/forge-ai/dist/`.
...
### Base Message Types (from forge-ai)
...
The exported forge-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before forge persists the assistant message, so `"pending"` should never appear in session JSONL.

### Extended Message Types (from forge-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if forge-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `forge.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `forge.setSessionName()` in extensions.
