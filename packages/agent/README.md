# @earendil-works/forge-agent-core

Stateful agent with tool execution and event streaming. Built on `@earendil-works/forge-ai`.

## Installation

```bash
npm install @earendil-works/forge-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@earendil-works/forge-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
```typescript
declare module "@earendil-works/forge-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/forge-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/forge-agent-core";
