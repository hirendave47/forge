...
describe("NodeExecutionEnv", () => {
	...
	it.each([
		...
		["an empty override shadows the base value", { FORGE_SESSION_FILE: "" }, "x:"],
		[
		...
		async (_description, overrides, expectedSessionFile) => {
			...
			const env = new NodeExecutionEnv({
				cwd: root,
				shellEnv: {
					FORGE_SESSION_FILE: "/stale/parent.jsonl",
					FORGE_CODING_AGENT: "true",
					FORGE_NODE_ENV_PRESERVED_TEST: "preserved",
				},
			});
			const result = getOrThrow(
				await env.exec(
					`printf '%s:%s|%s|%s' "\${FORGE_SESSION_FILE+x}" "\${FORGE_SESSION_FILE-}" "$FORGE_CODING_AGENT" "$FORGE_NODE_ENV_PRESERVED_TEST"`,
					{ env: overrides },
				),
			);
	...
	it("can replace rather than inherit the default shell environment", async () => {
		const root = createTempDir();
		const inheritedKey = "FORGE_NODE_ENV_INHERITED_TEST";
		const configuredKey = "FORGE_NODE_ENV_CONFIGURED_TEST";
		const explicitKey = "FORGE_NODE_ENV_EXPLICIT_TEST";
