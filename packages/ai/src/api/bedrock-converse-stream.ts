...
export const stream: StreamFunction<"bedrock-converse-stream", BedrockOptions> = (
	...
	(async () => {
		...
		// A profile explicitly configured through forge's auth flow (the `profile`
...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "forge-ai-custom-headers", priority: "low" });
}
...
function addResponseHeadersMiddleware(
	...
	client.middlewareStack.add(middleware, { step: "deserialize", name: "forge-ai-response-headers" });
}
 ...
 * Defaults to "short". Set FORGE_CACHE_RETENTION=long to enable long retention.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("FORGE_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
