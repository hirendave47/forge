...
function createClient(
	...
	const headers = providerHeadersToRecord({ "User-Agent": getForgeUserAgent(), ...model.headers, ...optionsHeaders });
	if (headers) {
...
function getDisabledThinkingConfig(model: Model<"google-generative-ai">): ThinkingConfig {
	...
	// thinkingLevel without includeThoughts so hidden thinking remains invisible to forge.
