...
const FORGE_USER_AGENT = `forge (${platform()} ${release()}; ${arch()})`;
...
describe("xAI Responses provider", () => {
	...
	it("uses /responses with bearer auth and xAI-compatible request fields", async () => {
		...
		expect(captured.headers.get("user-agent")).toBe(FORGE_USER_AGENT);
		expect(captured.headers.get("session_id")).toBe("forge-session-123");
		expect(captured.body).toMatchObject({
			model: "grok-4.5",
			store: false,
			stream: true,
			prompt_cache_key: "forge-session-123",
	...
	it("uses forge's User-Agent by default for Responses requests", async () => {
		let userAgent: string | null = null;
		vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
			userAgent = new Request(input, init).headers.get("user-agent");
			return completedResponse();
		...
		expect(result.stopReason, result.errorMessage).toBe("stop");
		expect(userAgent).toBe(FORGE_USER_AGENT);
	});
	...
	it("uses forge's User-Agent by default for Completions requests", async () => {
		expect(await captureCompletionsUserAgent()).toBe(FORGE_USER_AGENT);
	});
