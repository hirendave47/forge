...
```bash
npm install @earendil-works/forge-ai
```

TypeBox exports are re-exported from `@earendil-works/forge-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@earendil-works/forge-ai';
...
import { openaiProvider } from '@earendil-works/forge-ai/providers/openai';
import { openrouterProvider } from '@earendil-works/forge-ai/providers/openrouter';
import { amazonBedrockProvider } from '@earendil-works/forge-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@earendil-works/forge-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@earendil-works/forge-ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. forge-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@earendil-works/forge-ai';
...
API-key credentials use the same discriminator as forge's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@earendil-works/forge-ai';
...
```typescript
import { validateToolCall, type Tool } from '@earendil-works/forge-ai';
...
```typescript
import { builtinImagesModels } from '@earendil-works/forge-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@earendil-works/forge-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@earendil-works/forge-ai';
import { openAICompletionsApi } from '@earendil-works/forge-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@earendil-works/forge-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@earendil-works/forge-ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are forge thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@earendil-works/forge-ai/api/anthropic-messages';
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for forge-controlled thinking values
  chatTemplateArgs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort' | 'thinking.budget'; omitWhenOff?: boolean }>; // chat_template_args values for thinkingFormat: 'baseten'; use $var for forge-controlled thinking values
...
```typescript
import { createModels, type Context } from '@earendil-works/forge-ai';
import { anthropicProvider } from '@earendil-works/forge-ai/providers/anthropic';
import { openaiProvider } from '@earendil-works/forge-ai/providers/openai';
import { googleProvider } from '@earendil-works/forge-ai/providers/google';
...
- `@earendil-works/forge-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@earendil-works/forge-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@earendil-works/forge-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@earendil-works/forge-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@earendil-works/forge-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@earendil-works/forge-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@earendil-works/forge-ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `FORGE_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @earendil-works/forge-ai login              # interactive provider selection
npx @earendil-works/forge-ai login anthropic    # login to specific provider
npx @earendil-works/forge-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@earendil-works/forge-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@earendil-works/forge-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@earendil-works/forge-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@earendil-works/forge-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@earendil-works/forge-ai`.
