...
export class TuiMainScreen extends TuiBase implements TUI {
	...
	protected doRender(): void {
		...
		const debugRedraw = process.env.FORGE_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(this.logDirectory, "forge-debug.log");
		...
		// Configurable via setClearOnShrink() or FORGE_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(this.logDirectory, "forge-crash.log");
				const crashData = [
		...
		if (process.env.FORGE_TUI_DEBUG === "1") {
