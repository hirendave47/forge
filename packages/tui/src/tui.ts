...
export const VIEWPORT_TUI = Symbol.for("@earendil-works/forge-tui/viewport");
...
export abstract class TuiBase extends Container implements TUI {
	...
	private showHardwareCursor = process.env.FORGE_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.FORGE_CLEAR_ON_SHRINK === "1";
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory =
			logDirectory ?? process.env.FORGE_CODING_AGENT_DIR ?? path.join(os.homedir(), ".forge", "agent");
