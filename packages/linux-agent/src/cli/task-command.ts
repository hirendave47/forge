...
export async function handleTaskCommand(args: string[]): Promise<void> {
	...
	switch (subcommand) {
		...
		case "ai-wizard":
		case "ai":
			await handleAiWizard(args.slice(1));
			break;
		case "refine":
		case "ai-edit":
...
async function handleAiWizard(args: string[]): Promise<void> {
	const { runTaskArchitect } = await import("./task-architect/index.ts");
	let initialGoal: string | undefined;
	for (const arg of args) {
		if (!arg.startsWith("-") && !initialGoal) {
			initialGoal = arg;
		}
	}
	await runTaskArchitect({ initialGoal });
}

async function handleWizard(args: string[]): Promise<void> {
	let initialGoal: string | undefined;
	let smart: boolean | undefined;
	let useAiArchitect = false;
	for (const arg of args) {
		if (arg === "--ai") {
			useAiArchitect = true;
		} else if (arg === "--smart") {
...
${chalk.bold("Examples:")}
  ...
  forge task create --from tasks/nginx-monitor.yaml`);
	...
	if (useAiArchitect) {
		const { runTaskArchitect } = await import("./task-architect/index.ts");
		await runTaskArchitect({ initialGoal: goal });
		return;
	}
...
async function handleShow(args: string[]): Promise<void> {
	...
	try {
		...
		// Load bundle metadata for architecture explainability if present
		const { loadTaskBundle } = await import("./task-architect/index.ts");
		const bundle = loadTaskBundle(task.name);
		if (bundle?.manifest?.architecture) {
			const arch = bundle.manifest.architecture;
			console.log();
			console.log(chalk.bold("Architecture Explainability:"));
			console.log(`  Strategy:     ${arch.strategy}`);
			console.log(`  Scheduler:    ${arch.scheduler}`);
			console.log(`  Why Strategy: ${arch.explanation.why_strategy}`);
			console.log(`  Why Sched:    ${arch.explanation.why_scheduler}`);
			if (arch.explanation.estimated_ai_usage) {
				console.log(`  AI Usage:     ${arch.explanation.estimated_ai_usage}`);
			}
			if (bundle.scriptPath) {
				console.log(`  Fast Path:    ${bundle.scriptPath}`);
			}
			console.log(`  Generated By: ${arch.generated_by}`);
		}
	} finally {
		store.close();
	}
...
async function handleRefine(args: string[]): Promise<void> {
	const taskRef = args[0];
	if (!taskRef) {
		console.error(chalk.red("Error: Task ID or name required."));
		console.error(chalk.dim("Usage: forge task refine <task>"));
		process.exitCode = 1;
		return;
	}
	const { runTaskRefine } = await import("./task-architect/index.ts");
	const success = await runTaskRefine(taskRef);
	if (!success) {
		process.exitCode = 1;
	}
}
...
${chalk.bold("Commands:")}
  create "<goal>"           Create a new persistent task (--ai for AI design)
  ai-wizard [goal]          Interactive AI Task Architect to design and configure tasks
  refine <task>             Interactive AI refinement to evolve an existing task
  wizard                    Launch interactive task creation wizard (--ai for AI architect)
