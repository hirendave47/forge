...
interface SourceArtifact extends TestArtifactBase {
	type: "@earendil-works/forge-evals:source";
	runId: string;
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@earendil-works/forge-evals:session" &&
				artifact.type !== "@earendil-works/forge-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@earendil-works/forge-evals:session" ? "sessions" : "sources";
