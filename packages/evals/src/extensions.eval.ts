...
import { createForgeCodingAgentHarness, type ForgeCodingAgentInput } from "./forge-harness.ts";
...
function createExtensionAuthoringHarness(name: string, transformSystemPrompt?: (defaultPrompt: string) => string) {
	return createForgeCodingAgentHarness<ExtensionAuthoringOutput>({
		name,
...
const ExtensionAuthoringJudge = createJudge<ForgeCodingAgentInput, ExtensionAuthoringOutput>(
	...
	({ output, toolCalls }) => {
		...
		if (output.extensionSource === null) {
			...
			if (!imports.includes("@earendil-works/forge-coding-agent")) {
				failures.push("extension does not import the canonical @earendil-works/forge-coding-agent package");
			}
