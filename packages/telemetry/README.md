# @earendil-works/forge-telemetry
...
- [Forge Package Integration](#forge-package-integration)
...
```bash
npm install @earendil-works/forge-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@earendil-works/forge-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@earendil-works/forge-telemetry';
...
`@earendil-works/forge-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@earendil-works/forge-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@earendil-works/forge-agent-core';
...
- `@earendil-works/forge-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@earendil-works/forge-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@earendil-works/forge-agent-core` owns and exports the forge AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
import {
  ...
  startAiSpan,
  startHarnessSpan,
} from '@earendil-works/forge-agent-core';
```

The forge schemas use forge-owned `forge.ai.*`, `forge.harness.*`, and `forge.session.*` names. Adapters may translate them to backend conventions without changing the emitted forge vocabulary.
