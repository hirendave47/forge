...
const packages = [
	{ directory: "packages/telemetry", name: "@earendil-works/forge-telemetry" },
	{ directory: "packages/ai", name: "@earendil-works/forge-ai" },
	{ directory: "packages/tui", name: "@earendil-works/forge-tui" },
	{ directory: "packages/agent", name: "@earendil-works/forge-agent-core" },
	{ directory: "packages/protocol", name: "@earendil-works/forge-protocol" },
	{ directory: "packages/client", name: "@earendil-works/forge-client" },
	{ directory: "packages/session-backends/sqlite-node", name: "@earendil-works/forge-session-backend-sqlite-node" },
	{ directory: "packages/server", name: "@earendil-works/forge-server" },
	{ directory: "packages/coding-agent", name: "@earendil-works/forge-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "forge-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `forge-${platform}.zip` : `forge-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createForgeShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "forge.cmd"))) {
			writeFileSync(join(installDirectory, "forge.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\forge.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "forge.ps1"), '& "$PSScriptRoot/node_modules/.bin/forge.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "forge.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\forge.exe" %*\r\n');
		writeFileSync(join(installDirectory, "forge.ps1"), '& "$PSScriptRoot/node_modules/.bin/forge.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "forge"), join(installDirectory, "forge"));
}
...
if (rootPackageJson.name !== "forge-monorepo") {
...
if (!options.skipInstall) {
	...
	createForgeShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createForgeShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `forge-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "forge.exe" : "forge")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "forge.cmd" : "forge")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "forge.cmd" : "forge")} --help`);
	}
}
