#!/usr/bin/env bash
#
# Build forge binaries for all platforms locally.
...
#     forge-darwin-arm64.tar.gz
#     forge-darwin-x64.tar.gz
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        ...
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/forge"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating forge-$platform.zip..."
        (cd "$platform" && zip -r ../forge-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating forge-$platform.tar.gz..."
        mv "$platform" forge && tar -czf forge-$platform.tar.gz forge && mv forge "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../forge-$platform.zip)
    else
        tar -xzf forge-$platform.tar.gz && mv forge "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/forge.exe"
    else
        echo "  $OUTPUT_DIR/$platform/forge"
    fi
done
