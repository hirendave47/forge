...
const agentDirEnvName = "FORGE_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "FORGE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific FORGE_CODING_AGENT_DIR for the benchmark run
  ...
  --no-offline           Do not force FORGE_OFFLINE=1 / FORGE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	const env = { ...process.env, FORGE_TIMING: "1" };
	if (options.agentDir) {
	...
	if (options.offline) {
		env.FORGE_OFFLINE = "1";
		env.FORGE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "forge-startup-benchmark-")) : undefined;
