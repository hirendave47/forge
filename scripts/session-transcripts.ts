 ...
 *   --analyze      Spawn forge subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("forge", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
...
async function main() {
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn forge subagents for pattern analysis.");
		return;
	}
